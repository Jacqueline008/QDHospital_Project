...
class TreatmentController extends Controller {
    ...
    public function dutyShow(){
        ...
        //查询查该hos_code所对应的医院名称
        $hospital_Model=M("Hospital");
        $hos_name=$hospital_Model->where("hos_code='".$hos_code."'")->getField("hos_name");
        //将查出的信息传入视图层
        $this->assign("hos_name",$hos_name);

        //查询该hos_code dep_code所对应的科室信息
        $department_Model=M("Department");
...
//********************************************************************
        ...
        //获取禁用时长
        $dictionary_item_Model=M("DictionaryItem");
        $stop_time_value=$dictionary_item_Model->where("dic_item='STOP_TIME' and dic_code='1' ")->getField("dic_value");
        //将查询出的结果传入视图层
        $this->assign("stop_time_value",$stop_time_value);

        //如果患者登录了，并且在黑名单中，就获取end_date
        if(session("?userID")){
            $p_id=session("userID");
            $blacklist_Model=M("Blacklist");
            $end_date=$blacklist_Model->where("p_id='".$p_id."'")->getField("end_date");
            if($end_date!=null){
