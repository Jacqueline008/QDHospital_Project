...
<head>
    ...
    <!--css-this-->
    <link rel="stylesheet" href="/HOS/Public/libs/css/Hos/Dep/css_lst.css">
</head>
<body>
...
<div class="info-content-mobile">
    <!--部分1：搜索栏部分 begin-->
    <form class="ic-search-m" method="get" action="http://<?php echo C('IP_URL');?>/HOS/Hos/Dep/lst" target="_self" onsubmit="formSubmitMobile()">
        <!--下面的input name="dep_info"(当该input输入值时才提交到表单)-->
        <input class="search-item-m dep-info-m" type="text" placeholder="请输入科室代码/名称">
        <select class="search-item-m dic-value-m" name="dic_value">
            <option value="null">请选择科室类别</option>
            <?php if(is_array($dictionary_item_table)): $i = 0; $__LIST__ = $dictionary_item_table;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$dictionary_item_row): $mod = ($i % 2 );++$i;?><option value=<?php echo ($dictionary_item_row["dic_value"]); ?>><?php echo ($dictionary_item_row["dic_value"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
        </select>
        <div class="btn-container-m">
            <a class="btn-add-m" href="http://<?php echo C('IP_URL');?>/HOS/Hos/Dep/add"><img src="/HOS/Public/images/add.png"><span>添加</span></a>
    ...
    <div class="list-container-m">
        <?php if(is_array($department_table)): $i = 0; $__LIST__ = $department_table;if( count($__LIST__)==0 ) : echo "$empty" ;else: foreach($__LIST__ as $key=>$department_row): $mod = ($i % 2 );++$i;?><div class="list-item-m">
            <span class="dep_name-m"><?php echo ($department_row["dep_name"]); ?></span>
            <span class="dep_code-m"><?php echo ($department_row["dep_code"]); ?></span>
            <span class="dic_value-m"><span><?php echo ($department_row["dic_value"]); ?></span></span>
            <select class="btn-container-m">
                <option>操作</option>
                <option class="btn-edit-m" name="<?php echo ($department_row["dep_code"]); ?>">编辑</option>
                <option class="btn-del-m" name="<?php echo ($department_row["dep_code"]); ?>">删除</option>
            </select>
            <span class="dep_description-m"><?php echo ($department_row["small_description"]); ?>...</span>
...
<div id="info-container">
    ...
    <div class="info-list">
        ...
        <a href="http://<?php echo C('IP_URL');?>/HOS/Hos/Dep/lst" class="active"><img src="/HOS/Public/images/DepController.png"/>科室管理</a>
    ...
    <div class="info-content">
        <!--右侧1:搜索栏部分 begin-->
        <form class="ic-search" method="get" action="http://<?php echo C('IP_URL');?>/HOS/Hos/Dep/lst" target="_self" onsubmit="formSubmit()">
            <div class="search-item dep-info">
                <span>科室代码/名称</span>
                <!--下面的input name="dep_info"(当该input输入值时才提交到表单)-->
                <input type="text">
            </div>
            <div class="search-item dic-value">
                <span>科室类别</span>
                <select name="dic_value">
                    <option value="null">请选择科室类别</option>
                    <?php if(is_array($dictionary_item_table)): $i = 0; $__LIST__ = $dictionary_item_table;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$dictionary_item_row): $mod = ($i % 2 );++$i;?><option value=<?php echo ($dictionary_item_row["dic_value"]); ?>><?php echo ($dictionary_item_row["dic_value"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                </select>
            </div>
            <div class="btn-container">
                <a class="btn-add" href="http://<?php echo C('IP_URL');?>/HOS/Hos/Dep/add"><img src="/HOS/Public/images/add.png"><span>添加</span></a>
        ...
        <div class="list-container">
            <div class="list-head">
                <span class="dep_code">科室代码</span>
                <span class="dep_name">科室名称</span>
                <span class="dic_value">科室类别</span>
                <span class="dep_description">科室简介</span>
                <div class="btn-container"></div>
            </div>
            <?php if(is_array($department_table)): $i = 0; $__LIST__ = $department_table;if( count($__LIST__)==0 ) : echo "$empty" ;else: foreach($__LIST__ as $key=>$department_row): $mod = ($i % 2 );++$i;?><div class="list-item">
                    <span class="dep_code"><?php echo ($department_row["dep_code"]); ?></span>
                    <span class="dep_name"><?php echo ($department_row["dep_name"]); ?></span>
                    <span class="dic_value"><?php echo ($department_row["dic_value"]); ?></span>
                    <span class="dep_description"><?php echo ($department_row["small_description"]); ?>...</span>
                    <div class="btn-container">
                        <div class="btn-del" name="<?php echo ($department_row["dep_code"]); ?>"><img src="/HOS/Public/images/del-orange.png"><span>删除</span></div>
                        <div class="btn-edit" name="<?php echo ($department_row["dep_code"]); ?>"><img src="/HOS/Public/images/edit-blue.png"><span>编辑</span></div>
                    </div>
                </div><?php endforeach; endif; else: echo "$empty" ;endif; ?>
        </div>
        <!--右侧2:列表部分 end-->
...
<script>
    //科室管理-编辑页面的地址
    var editUrl="http://<?php echo C('IP_URL');?>/HOS/Hos/Dep/edit/";
    //科室管理-删除页面的地址
    var delUrl="http://<?php echo C('IP_URL');?>/HOS/Hos/Dep/del/";
</script>
<script src="/HOS/Public/libs/js/Hos/Dep/js_lst.js"></script>
